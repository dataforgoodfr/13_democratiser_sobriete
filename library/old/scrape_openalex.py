...
from library.connectors.openalex.old.parse_metadata import (
    # get_all_results,
    # remove_unnecessary_fields,
    clean_result_fields,
)
from library.connectors.openalex.old.extract_openalex import get_urls_to_fetch, search_openalex
...
def download_pdf(url: str, driver: webdriver.Chrome, output_file_path: str, maxwait=30) -> str:
    def get_last_downloaded_file_path(dummy_dir) -> str:
        ...
        while not os.listdir(dummy_dir):
            time.sleep(10)
        return max(
            [os.path.join(dummy_dir, f) for f in os.listdir(dummy_dir)], key=os.path.getctime
        )
...
def scrape_all_urls(
    ...
    all_files = set(
        f.replace(".pkl", "") for f in os.listdir(os.path.join(output_dir, "pkl_files"))
    )
    ...
    scrapped_files = {
        f: {
            ...
            if os.path.isfile(pdf_path := os.path.join(output_dir, "pdf_files", f) + ".pdf")
            else None,
        }
        for f in all_files
    }
    ...
    while (
        parsed_files < stop_criterion
    ):  # TODO : check if this is the correct condition or merge with the one below
        ...
        if from_dois:
            assert (
                per_page <= 100
            ), "Can only call the API with up to 100 DOIs in a single call, set per_page to 100 or lower"
            sub_dois = dois[doi_idx : doi_idx + per_page]
        else:
            sub_dois = []
        query_data = search_openalex(
            query,
            cursor=cursor,
            per_page=per_page,
            from_dois=from_dois is not None,
            dois=sub_dois,
        )
        ...
        failed_downloads, successfull_downloads, scrapped_files = scrape_list_urls(
            urls_to_fetch,
            filenames,
            driver,
            scrapped_files,
...
def get_papers(
    query_requested: str, limit: Optional[int] = None, dois: Optional[str] = DOI_PATH
) -> List[OpenAlexPaper]:
    # TODO : Expose usage of other parameters
    raw_papers_paths_dict = main(
        query=query_requested, stop_criterion=limit, from_dois=DOI_PATH
    )
    papers = []
