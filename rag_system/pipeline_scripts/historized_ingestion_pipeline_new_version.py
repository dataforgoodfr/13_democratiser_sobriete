...
class IndexingPipelineShortCut(IndexPipeline):
    ...
    def handle_docs(self, docs, file_id, file_path, article_metadata) -> int:
        ...
        try:
            ...
            metadatas_json = self.concat__metadatas_layer(metadatas_base=metadatas_json,
                                                          metadatas_root=METADATA_BASE)
            ...
            temporary_chunk_metadata = self.enrich_metadatas_layer(metadatas_base=METADATA_BASE,
        ...
        try:
            text_vs_metadatas = [temporary_chunk_metadata for _ in range(len(thumbnail_docs) + len(non_text_docs))] #temporary
            ...
            logging.info(f"Got {len(thumbnail_docs)} text chunks - {len(thumbnail_docs)} page thumbnails - {len(non_text_docs)} other type chunks - 1 entire doc")
...
def main():
    ...
    indexing_pipeline.run_one_file(file_path=file_path,
