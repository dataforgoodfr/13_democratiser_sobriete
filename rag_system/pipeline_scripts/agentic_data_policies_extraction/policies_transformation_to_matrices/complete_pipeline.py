...
def run_complete_pipeline(limit: int = 100, sim_threshold: float = 0.75):
    ...
    try:
        ...
        if "cluster_id" in flattened_df.columns and flattened_df["cluster_id"].nunique() > 1:
            ...
            cluster_summary = (
                flattened_df.groupby(["cluster_id", "policy_canonical"])
                .size()
        ...
        summary_report = {
            ...
            "clusters": flattened_df["cluster_id"].nunique()
            if "cluster_id" in flattened_df.columns
            else 0,
            "correlation_distribution": flattened_df["correlation_numeric"]
            .value_counts()
            .to_dict(),
        }
...
if __name__ == "__main__":
    ...
    if result_df is not None:
        ...
        print(
            f"Results saved to: complete_flattened_policies.csv, complete_policy_sector_matrix.csv"
        )
    else:
