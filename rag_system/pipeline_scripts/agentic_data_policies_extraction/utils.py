...
def fetch_policies_with_texts(db_client: DatabaseClient, qdrant_client: QdrantClient, limit: int = 10, offset: int = 0) -> List[Dict[str, Any]]:
    """
...
def get_policy_text(qdrant_client: QdrantClient, policy: Dict[str, Any]) -> str:
    ...
    if qdrant_data:
        logger.info(f"  Found text in Qdrant with payload keys: {list(qdrant_data['payload'].keys())}")
        ...
        if 'text' in qdrant_data['payload']:
            ...
            logger.info(f"  No 'text' field found in payload. Available fields: {list(qdrant_data['payload'].keys())}")
    else:
