...
# DOCSTORE_PATH = "/app/ktem_app_data/user_data/docstore" !!! Now, defined in the flowsettings.py (or in kotaemon for s3 path)


COLLECTION_ID = 7  # collection id is the integer at the end of the collection 'index_[ID] '
USER_ID = "2*************************************"  # ! => Open the Kotaemon app ... go on a random page... see the logs to retrieve the USER ID !
...
qdrant_host = "116919ed-8e07-47f6-8f24-a22527d5d520.europe-west3-0.gcp.cloud.qdrant.io"  #  ! to report in flowsetting.py !
...
class IndexingPipelineShortCut(IndexPipeline):
    ...
    file_index_associated = FileIndex(
        ...
        config={
            "embedding": "default",
            "supported_file_types": ".png, .jpeg, .jpg, .tiff, .tif, .pdf, .xls, .xlsx, .doc, .docx, .pptx, .csv, .html, .mhtml, .txt, .md, .zip",
            "max_file_size": 1000,
            ...
            if reapply_fields_to_root is not None:
                for field in reapply_fields_to_root:
                    if field not in inheritance_metadatas.keys():
                        logging.warning(
                            f"Sorry, but the field {field} is not present in inheritance metadatas for reapplying :  {inheritance_metadatas.keys()}"
                        )
                    else:
    ...
    def handle_docs(self, docs, file_id, file_path, article_metadata) -> int:
        s_time = time.time()
        ...
        try:
            ...
            # ------------ CUSTOM LOGIC ---------------------
            # *** Inference metadatas for entire doc ***
            ...
            metadatas_json["doc_type"] = "entire_doc"
            metadatas_json = self.concat__metadatas_layer(
                metadatas_base=metadatas_json, metadatas_root=METADATA_BASE
            )

            entire_doc = Document(text=text_md, id_=str(uuid.uuid4()))
            # Clone metadata first doc => entire doc (not OpenAlex or llm metadata !... it's relative to internal metadata sqlite db system)
            ...
            temporary_chunk_metadata = self.enrich_metadatas_layer(
                ...
                inheritance_metadatas=metadatas_json,
        ...
        try:
            ...
            to_index_metadatas = text_vs_metadatas + other_vs_metadatas + [metadatas_json]

            logging.info(
                f"Got {len(thumbnail_docs)} text chunks - {len(thumbnail_docs)} page thumbnails - {len(non_text_docs)} other type chunks - 1 entire doc"
            )
    ...
    def run_one_file(self, file_path: str | Path, reindex: bool = False, **kwargs) -> None:
        tic = time.time()
        ...
        try:
            ...
            if isinstance(file_path, Path):
                ...
                article_metadata = get_open_alex_article(file_path)
            ...
            nb_chunks = self.handle_docs(docs, file_id, file_path, article_metadata)
...
def main():
    ...
    indexing_pipeline.run_one_file(file_path=file_path, reindex=args.force_reindex)
