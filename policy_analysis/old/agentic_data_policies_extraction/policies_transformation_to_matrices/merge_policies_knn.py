...
# -------- Main pipeline --------
def merge_policies_semantic_medoid(
    df: pd.DataFrame,
    text_col: str = None,
    model_name: str = None,
    ...
    try:
        ...
        normalize_text_flag = (
            normalize_text_flag if normalize_text_flag is not None else CFG.normalize_text_flag
        )
        ...
        print(
            f"Building FAISS k-NN (k={k}) and clustering with threshold={sim_threshold:.2f}..."
        )
        ...
        for cid, grp in df.groupby("cluster_id", sort=False):
            ...
            if len(grp_emb_indices) == 1:
                ...
                medoid_original_idx = choose_medoid_indices(
                    grp_emb_indices, embs
                )  # Already an index in grp.index
...
if __name__ == "__main__":
    try:
        ...
        # Quick view
        summary = (
            out.groupby(["cluster_id", "policy_canonical"])
            .size()
