#!/bin/bash
#SBATCH --job-name=GeoExtraction # nom du job
#SBATCH --output=GeoExtraction%j.out # fichier de sortie (%j = job ID)
#SBATCH --error=GeoExtraction%j.err # fichier d’erreur (%j = job ID)
...
srun python vllm_geo_extraction.py
