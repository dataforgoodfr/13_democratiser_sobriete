"""Prompt v2 for the sufficiency classifier — grounded in the six-pillar definition.
...
PROMPT_VERSION = "v2-sixpillar-5class-10sub"
...
P2 — ALL NATURAL RESOURCES. Demand for energy, water, land, space, materials,
   ...
   measure just because the resource is not energy. Conversely, a CARBON-ONLY
   move that leaves physical throughput unchanged (offsets, carbon capture,
   afforestation-as-offset) is NOT sufficiency.
...
WHEN NOT TO CLASSIFY AS A POLICY:
  ...
  it into a category. Use `ambiguous` when it IS a policy but the mechanism is
  genuinely mixed or no single category dominates.
"""
...
DECISION PROCEDURE (follow in order):
  ...
  6. If genuinely mixed or undominated → ambiguous.
