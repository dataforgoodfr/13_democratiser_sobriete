#!/bin/bash
#SBATCH --job-name=PolicyExtraction # nom du job
#SBATCH --output=PolicyExtraction%j.out # fichier de sortie (%j = job ID)
#SBATCH --error=PolicyExtraction%j.err # fichier d’erreur (%j = job ID)
...
srun python vllm_policy_extraction.py