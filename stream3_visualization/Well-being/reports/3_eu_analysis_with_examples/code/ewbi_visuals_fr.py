...
# Suppress Tkinter warnings from kaleido's image rendering
warnings.filterwarnings("ignore", category=RuntimeWarning, message=".*main thread.*")
...
TRANSLATIONS = {
    'en': {
        'european_well_being_index': 'European Well-Being Index (EWBI)',
        'france_vs_eu27_comparison': 'France vs EU-27 Comparison',
        'year': 'Year',
        'ewbi_score': 'EWBI Score',
        'score': 'Score',
        'france_ewbi': 'France EWBI',
        'eu27_ewbi': 'EU-27 EWBI',
        'france': 'France',
        'eu27': 'EU-27',
        'income_decile': 'Income Decile',
        'france_avg': 'France Avg',
        'eu27_avg': 'EU-27 Avg',
        'energy_and_housing': 'Energy and Housing',
        'overall': 'Overall',
        'deciles': 'by Income Decile',
    },
    'fr': {
        'european_well_being_index': 'Indice de Bien-Être Européen (IBEE)',
        'france_vs_eu27_comparison': 'Comparaison France vs UE-27',
        'year': 'Année',
        'ewbi_score': 'Score IBEE',
        'score': 'Score',
        'france_ewbi': 'France IBEE',
        'eu27_ewbi': 'UE-27 IBEE',
        'france': 'France',
        'eu27': 'UE-27',
        'income_decile': 'Décile de Revenu',
        'france_avg': 'Moy. France',
        'eu27_avg': 'Moy. UE-27',
        'energy_and_housing': 'Énergie et Logement',
        'overall': 'Ensemble',
        'deciles': 'par Décile de Revenu',
    }
}
...
def get_app_level5_indicators():
    ...
    for eu_priority in EU_PRIORITIES:
        ...
        for indicator in primary_options:
            display_name = get_display_name(indicator, language='fr')
...
def create_ewbi_overall_comparison_plot(france_ewbi_overall, eu27_ewbi_overall, output_dir, language='en'):
    ...
    if not france_ewbi_overall.empty:
        fig.add_trace(go.Scatter(
            ...
            mode='lines+markers',
            name=get_translation('france_ewbi', language),
            line=dict(color=FRANCE_COLOR, width=4),
            marker=dict(color=FRANCE_COLOR, size=10, symbol='circle'),
            hovertemplate='<b>' + get_translation('france_ewbi', language) + '</b><br>' +
                         get_translation('year', language) + ': %{x}<br>' +
    ...
    if not eu27_ewbi_overall.empty:
        fig.add_trace(go.Scatter(
            ...
            mode='lines+markers',
            name=get_translation('eu27_ewbi', language),
            line=dict(color=EU_27_COLOR, width=4),
            marker=dict(color=EU_27_COLOR, size=10, symbol='circle'),
            hovertemplate='<b>' + get_translation('eu27_ewbi', language) + '</b><br>' +
                         get_translation('year', language) + ': %{x}<br>' +
                         'Score: %{y:.3f}<extra></extra>'
        ))
    ...
    fig.update_layout(
        title=dict(
            text=f"{get_translation('european_well_being_index', language)}<br><sub>{get_translation('france_vs_eu27_comparison', language)} ({int(earliest_year)}-{int(latest_year)})</sub>",
        ...
        yaxis=dict(
            title=get_translation('ewbi_score', language),
            title_font=dict(size=14, family="Arial, sans-serif"),
            tickfont=dict(size=12, family="Arial, sans-serif"),
            showgrid=True,
            gridcolor='lightgray',
    ...
    png_path = os.path.join(output_dir, f"France_vs_eu27_ewbi_overall{lang_suffix}.png")
    try:
        ...
        if not france_ewbi_overall.empty:
            plt.plot(france_ewbi_overall['Year'], france_ewbi_overall['Value'], 
                    ...
                    markersize=10, label=get_translation('france_ewbi', language), alpha=0.9)
        
        if not eu27_ewbi_overall.empty:
            plt.plot(eu27_ewbi_overall['Year'], eu27_ewbi_overall['Value'], 
                    color=EU_27_COLOR, linewidth=4, marker='o', 
                    markersize=10, label=get_translation('eu27_ewbi', language), alpha=0.9)
        
        plt.title(f"{get_translation('european_well_being_index', language)}\n{get_translation('france_vs_eu27_comparison', language)} ({int(earliest_year)}-{int(latest_year)})", 
                 fontsize=18, fontweight='bold', pad=20)
        plt.xlabel(get_translation('year', language), fontsize=14)
        plt.ylabel(get_translation('ewbi_score', language), fontsize=14)
...
def create_ewbi_deciles_comparison_plot(france_ewbi_deciles, eu27_ewbi_deciles, output_dir):
    ...
    if not france_latest.empty:
        ...
        fig.add_trace(go.Bar(
            x=france_x,
            y=france_y,
            name=get_translation('france_ewbi', 'fr'),
    ...
    if not eu27_latest.empty:
        ...
        fig.add_trace(go.Bar(
            x=eu27_x,
            y=eu27_y,
            name=get_translation('eu27_ewbi', 'fr'),
    ...
    fig.update_layout(
        title=dict(
            text=f"{get_translation('european_well_being_index', 'fr')}<br><sub>France vs UE-27 {get_translation('deciles', 'fr')} ({int(latest_year)})</sub>",
            font=dict(size=18, color="#2C3E50", family="Arial, sans-serif"),
            x=0.5
        ),
        xaxis=dict(
            title=get_translation('income_decile', 'fr'),
            title_font=dict(size=14, family="Arial, sans-serif"),
            tickfont=dict(size=12, family="Arial, sans-serif"),
            showgrid=True,
            gridcolor='lightgray',
        ...
        yaxis=dict(
            title=get_translation('ewbi_score', 'fr'),
            title_font=dict(size=14, family="Arial, sans-serif"),
            tickfont=dict(size=12, family="Arial, sans-serif"),
            showgrid=True,
            gridcolor='lightgray',
    ...
    try:
        ...
        plt.title(f"{get_translation('european_well_being_index', 'fr')}\nFrance vs UE-27 {get_translation('deciles', 'fr')} ({int(latest_year)})", 
                 fontsize=18, fontweight='bold', pad=20)
...
def create_eu_priority_overall_comparison_plot(france_data, eu27_data, priority_name, output_dir):
    ...
    try:
        ...
        plt.title(f"{priority_name}\nComparaison France vs UE-27 ({int(earliest_year)}-{int(latest_year)})", 
                 fontsize=16, fontweight='bold', pad=20)
        plt.xlabel(get_translation('year', 'fr'), fontsize=14)
        plt.ylabel(get_translation('score', 'fr'), fontsize=14)
        plt.grid(True, alpha=0.3)
        plt.legend(fontsize=12, loc='best')
...
def create_eu_priority_deciles_comparison_plot(france_data, eu27_data, france_overall, eu27_overall, priority_name, output_dir):
    ...
    try:
        ...
        plt.title(f"{priority_name}\nFrance par Décile de Revenu vs UE-27 ({int(latest_year)})", 
                 fontsize=14, fontweight='bold', pad=15)
        plt.xlabel(get_translation('income_decile', 'fr'), fontsize=12, fontweight='bold')
        plt.ylabel(get_translation('score', 'fr'), fontsize=12, fontweight='bold')
...
def create_france_eu_priority_overall_plot(priority_data, priority_name, output_dir):
    ...
    try:
        ...
        plt.title(f"{priority_name} - France\nÉvolution du Score Global ({int(earliest_year)}-{int(latest_year)})", 
                 fontsize=16, fontweight='bold', pad=20)
        plt.xlabel(get_translation('year', 'fr'), fontsize=14)
        plt.ylabel(get_translation('score', 'fr'), fontsize=14)
        plt.grid(True, alpha=0.3)
...
def create_france_eu_priority_deciles_plot(priority_deciles, priority_name, output_dir):
    ...
    try:
        ...
        plt.title(f"{priority_name} - France\nPar Décile de Revenu ({int(latest_year)})", 
                 fontsize=16, fontweight='bold', pad=20)
        plt.xlabel(get_translation('income_decile', 'fr'), fontsize=14)
        plt.ylabel(get_translation('score', 'fr'), fontsize=14)
        plt.grid(True, alpha=0.3, axis='y')
        plt.legend(fontsize=12)
...
def create_level5_overall_plot(france_data, eu27_data, indicator, output_dir):
    ...
    if not france_indicator.empty:
        fig.add_trace(go.Scatter(
            ...
            mode='lines+markers',
            name=get_translation('france', 'fr'),
            line=dict(color=FRANCE_COLOR, width=4),
            marker=dict(color=FRANCE_COLOR, size=10, symbol='circle'),
            hovertemplate='<b>France</b><br>Année: %{x}<br>Valeur: %{y:.3f}<extra></extra>'
        ))
    ...
    # Get display name
    description = get_display_name(indicator, language='fr')
    
    # Update layout
    fig.update_layout(
        title=dict(
            text=f"{description}<br><sub>France vs UE-27 au fil du temps</sub>",
            font=dict(size=16, color="#2C3E50", family="Arial, sans-serif"),
            x=0.5
        ),
        xaxis=dict(
            title=get_translation('year', 'fr'),
    ...
    html_path = os.path.join(output_dir, f"level5_overall_{indicator}.html")
    try:
        fig.write_html(html_path)
        print(f"  [OK] Saved: {os.path.basename(html_path)}")
    except Exception as e:
...
def create_level5_deciles_plot(france_decile_data, eu27_decile_data, indicator, output_dir):
    ...
    if france_decile_data.empty:
        print(f"    DEBUG: france_decile_data is empty")
        return
    ...
    print(f"    DEBUG: france_deciles shape: {france_deciles.shape}, eu27_deciles shape: {eu27_deciles.shape}")

    if france_deciles.empty:
        print(f"    DEBUG: No France decile data for {indicator}")
        return

    # Get latest year
    latest_year = france_deciles['Year'].max()
    france_latest = france_deciles[france_deciles['Year'] == latest_year].copy()
    print(f"    DEBUG: Latest year: {latest_year}, France records at that year: {len(france_latest)}")

    # Convert decile to numeric and sort
    france_latest['Decile_num'] = pd.to_numeric(france_latest['Decile'], errors='coerce')
    france_latest = france_latest.dropna(subset=['Decile_num'])
    france_latest = france_latest.sort_values('Decile_num')
    print(f"    DEBUG: France after decile conversion: {len(france_latest)} records, deciles: {sorted(france_latest['Decile_num'].unique())}")

    if france_latest.empty:
        print(f"    DEBUG: France latest year data is empty after decile conversion")
        return
    ...
    try:
        # Create Plotly figure for deciles bar chart
        all_deciles = sorted(france_latest['Decile_num'].unique())
        decile_labels = [str(int(d)) for d in all_deciles]
        
        # Build France values list by iterating through deciles to ensure proper alignment
        france_values = []
        for decile in all_deciles:
            decile_data = france_latest[france_latest['Decile_num'] == decile]
            if not decile_data.empty:
                france_values.append(decile_data['Value'].iloc[0])
            else:
                france_values.append(None)
        ...
        fig.add_trace(go.Bar(
            ...
            name=get_translation('france', 'fr'),
            marker=dict(color=FRANCE_COLOR),
            text=[f'{v:.2f}' if v is not None else 'N/A' for v in france_values],
            textposition='outside'
        ))
        
        # Overlay EU-27 line (if available)
        if not eu27_latest.empty:
            eu27_values = []
            for decile in all_deciles:
                decile_data = eu27_latest[eu27_latest['Decile_num'] == decile]
                if not decile_data.empty:
                    eu27_values.append(decile_data['Value'].iloc[0])
                else:
                    eu27_values.append(None)
            
            if any(v is not None for v in eu27_values):
                fig.add_trace(go.Scatter(
                    x=decile_labels,
                    y=eu27_values,
                    name=get_translation('eu27', 'fr'),
        ...
        # Update layout
        fig.update_layout(
            title=f"{description}<br><sub>France vs UE-27 {get_translation('deciles', 'fr')} ({int(latest_year)})</sub>",
            xaxis=dict(title=get_translation('income_decile', 'fr')),
            yaxis=dict(title="Part de la population (%)"),
            plot_bgcolor='white',
            paper_bgcolor='white',
            hovermode='x unified',
            height=600,
        ...
        # Save as HTML
        html_path = os.path.join(output_dir, f"level5_deciles_{indicator}.html")
        fig.write_html(html_path)
        print(f"  [OK] Saved: {os.path.basename(html_path)}")
    except Exception as e:
...
def main():
    ...
    # Create EWBI_FR-specific subfolders under outputs/graphs (French version)
    ewbi_base = os.path.join(output_dir, 'EWBI_FR')
    ...
    # Create EWBI_FR tables folder structure
    tables_dir = os.path.abspath(os.path.join(report_dir, 'outputs', 'tables'))
    tables_ewbi_dir = os.path.join(tables_dir, 'EWBI_FR')
