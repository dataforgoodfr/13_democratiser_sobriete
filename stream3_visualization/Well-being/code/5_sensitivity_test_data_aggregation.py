...
# -*- coding: utf-8 -*-
"""
Sensitivity Analysis Script - Monte Carlo Experiments (Data Aggregation)

This script performs Monte Carlo sensitivity analysis on the EWBI computation pipeline
focusing on methodological variables (normalization, aggregation methods):
...
Output:
- Data files for each experiment in output/5_sensitivity_test_data_aggregation/data/
- No graphs generated (handled by 5_sensitivity_test_visuals_data_aggregation.py)
...
import json
import warnings
import matplotlib.pyplot as plt  # Not needed - visuals handled by separate script
...
def _set_experiment_env(output_dir: Path, config: dict) -> None:
    ...
    Pipeline stages read these at import time.
    This avoids rewriting stage files and prevents clashes with the app pipeline.
    """
    ...
    # Ensure EU-27 exists for experiment comparisons/plots.
...
def copy_input_data_for_sensitivity():
    ...
    if (OUTPUT_DIR / "0_raw_data_EUROSTAT").exists():
        ...
        if not dest_raw_data.exists():
            ...
            print(f"[SKIP] Raw EUROSTAT data already exists in sensitivity directory")
    else:
        print(f"[WARN] Raw EUROSTAT data not found in {OUTPUT_DIR / '0_raw_data_EUROSTAT'}")

    # Stage 1 outputs are the minimal required inputs for Stage 2/3.
    # In some repos the raw Stage 0 merge products may not be present, but a valid
    # Stage 1 output exists from a previous baseline run. Seed it into the
    # sensitivity pipeline output so Stage 3/4 can run.
    stage1_src = OUTPUT_DIR / "1_missing_data_output"
    stage1_dest = SENSITIVITY_STAGE1_OUTPUT
    stage1_dest.mkdir(parents=True, exist_ok=True)

    stage1_required = [
        "raw_data_break_adjusted.csv",
        # Helpful to keep continuity; Stage 3 will overwrite in sensitivity output anyway.
        "raw_data_forward_filled.csv",
    ]

    for name in stage1_required:
        src_path = stage1_src / name
        dest_path = stage1_dest / name
        if dest_path.exists():
            continue
        if src_path.exists():
            shutil.copy2(src_path, dest_path)
            print(f"[COPIED] Stage 1 seed input: {name}")
        else:
            print(f"[WARN] Stage 1 seed file not found: {src_path}")
...
def validate_protected_directories():
    """
    Validate that protected directories and files are never modified during sensitivity tests.
    """
    ...
    for protected_dir in PROTECTED_DIRS:
        if protected_dir.exists():
            ...
            protected_states[str(protected_dir)] = os.path.getmtime(protected_dir)
...
def check_protected_directories(initial_states):
    ...
    for protected_dir in PROTECTED_DIRS:
        if protected_dir.exists():
            current_mtime = os.path.getmtime(protected_dir)
...
# ===============================
# PARAMETER OPTIONS - METHODOLOGICAL VARIABLES EXPERIMENT
# ===============================
PARAMETER_OPTIONS = {
    # Stage 3: Normalization - Methodological choices
    'NORMALIZATION_METHOD': ['zscore', 'percentile'],
    'RESCALE_MIN': [0.05, 0.1, 0.2],
    'NORMALIZATION_APPROACH': ['multi_year', 'per_year'],
    # Stage 4: Aggregation - Methodological choices
    'PCA_SCOPE': ['all_years', 'per_year'],
    'EU_PRIORITIES_APPROACH': ['pca', 'simple'],
...
BASELINE_CONFIG = {
    # Data treatment variables (fixed at baseline)
    'BREAK_THRESHOLD': 0.2,
    'APPLY_MOVING_AVERAGE': True,
    'MOVING_AVERAGE_WINDOW': 5,
    'APPLY_MEAN_RESCALING': False,
    # Methodological variables (will be varied in experiments)
...
def run_pipeline_stage(stage_name, module_name):
    ...
    with redirect_stdout(f):
        try:
            ...
            # Re-raise with stage output attached.
...
def run_full_pipeline(config, experiment_id):
    ...
    # Configure stage scripts via env vars and isolate all outputs.
    _set_experiment_env(SENSITIVITY_PIPELINE_OUTPUT, {
        **BASELINE_CONFIG,
        **config,
    })
    
    # Expected outputs for each stage (in sensitivity pipeline output)
    ...
    # Run pipeline stages
    try:
        # Stage 1/2 are deterministic for this sensitivity run (they don't vary with the
        # parameters we sample here). Skip if already present to avoid unnecessary reruns.
        if expected_stage1.exists():
            print(f"[SKIP] Stage 1 output already present: {expected_stage1.name}")
        else:
            ...
            if not expected_stage1.exists():
                raise FileNotFoundError(
                    ...
                    "Check that raw inputs are available (or seed Stage 1 outputs)."
                )

        if expected_stage2.exists():
            print(f"[SKIP] Stage 2 output already present: {expected_stage2.name}")
        else:
            run_pipeline_stage("Stage 2: Multivariate Analysis", "2_multivariate_analysis")
            if not expected_stage2.exists():
                raise FileNotFoundError(
                    f"Stage 2 completed but did not produce {expected_stage2}. "
                    "Stage 4 may fall back to unweighted aggregation without this file."
                )

        # Stage 3/4 depend on the sampled methodological parameters; always rerun.
    ...
    possible_paths = [
        # Stage 4 output location
        SENSITIVITY_STAGE4_OUTPUT / "ewbi_final_aggregated.csv",
        # Stage 4 also writes an app-ready copy at the pipeline output root
        SENSITIVITY_PIPELINE_OUTPUT / "ewbi_master_aggregated.csv",
    ]
...
def run_monte_carlo_experiments():
    """
    Generate and run Monte Carlo sensitivity experiments.
    
    This function:
    1. Samples random parameter configurations
    2. Runs the full pipeline for each experiment  
    3. Saves results for later analysis
    """
    print("\n" + "="*70)
    print("MONTE CARLO SENSITIVITY EXPERIMENTS - GENERATION")
    print("="*70)
    print(f"\nConfiguration:")
    ...
    # CRITICAL PROTECTION: Validate protected directories before starting
    print("\n" + "="*70)
    print("PROTECTION VALIDATION")
    print("="*70)
    initial_protected_states = validate_protected_directories()
    ...
    try:
        ...
        for exp_num in range(1, N_EXPERIMENTS + 1):
            ...
            try:
                ...
                # Add fixed baseline parameters that don't vary
                full_config = {**BASELINE_CONFIG, **config}
                
                print(f"Sampled configuration:")
                print(config_to_string(config))
        ...
        if successful_experiments > 0:
            print(f"\n✓ Experiment data generated successfully")
            ...
            print(f"  2. Generate visualizations: python 5_sensitivity_test_visuals_data_aggregation.py")
        else:
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--eu27':
        ...
        print("Monte Carlo Sensitivity Analysis Script")
        print("=" * 50)
        print("Usage:")
        print("  python 5_sensitivity_test_data_aggregation.py            # Generate experiments")
        print("  python 5_sensitivity_test_data_aggregation.py --eu27     # Add EU-27 data")
        print("  python 5_sensitivity_test_data_aggregation.py --help     # Show this help")
        print()
        print("Modes:")
        ...
        print("Monte Carlo Sensitivity Analysis - Experiment Generation")
        print("=" * 60)
        print("This will generate sensitivity analysis experiments.")
        ...
        if len(existing_experiments) > 0:
            ...
            if response not in ['y', 'yes']:
                ...
                print("Regenerating all experiments...")
