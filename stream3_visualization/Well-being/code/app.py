...
try:
    ...
    print(
        f"Dashboard initialized with {len(EU_PRIORITIES)} EU priorities and {len(SECONDARY_INDICATORS)} secondary indicators")
...
def create_primary_indicator_charts(map_df, analysis_df, time_df, eu_priority, secondary_indicator, primary_indicator):
    ...
    if primary_col in analysis_df.columns:
        ...
        # Then add any other selected countries (excluding EU Countries Average to avoid duplication)
        other_countries = [c for c in analysis_df['country'].unique() if
                           c != 'EU Countries Average' and 'Average' not in c]
        countries_to_show.extend(other_countries)
    ...
    try:
        ...
        if 'primary_index' in time_series_df.columns:
            ...
            if not indicator_time_data.empty:
                ...
                country_year_data = indicator_time_data.groupby(['country', 'year'])[
                    'primary_score'].mean().reset_index()
...
def create_level_filters(eu_priority, secondary_indicator, primary_indicator):
    ...
    elif eu_priority != 'ALL' and secondary_indicator != 'ALL' and (
            primary_indicator == 'ALL' or not primary_indicator):
...
def create_adaptive_map_chart(map_df, level_filters):
    ...
    print(
        f"DEBUG: Filters - EU Priority: {level_filters['eu_priority']}, Secondary: {level_filters['secondary_indicator']}, Primary: {level_filters['primary_indicator']}")
...
def create_adaptive_decile_chart(analysis_df, level_filters):
    ...
    for country in countries_to_show:
        if country in filtered_data['country'].values:
            ...
            country_data = country_data.sort_values('decile_str',
                                                    key=lambda x: pd.Categorical(x, categories=decile_order,
                                                                                 ordered=True))
...
def create_adaptive_radar_country_chart(analysis_df, level_filters):
    ...
    print(
        f"DEBUG: Creating radar/country chart for Level {level_filters['current_level']}: {level_filters['level_name']}")
...
def create_adaptive_time_series_chart(analysis_df, level_filters, selected_countries=None):
    ...
    print(
        f"DEBUG: Creating time series chart for Level {level_filters['current_level']}: {level_filters['level_name']}")
