...
def load_baseline_data():
    ...
    primary_path = SENSITIVITY_DATA / "experiment_0_baseline_ewbi.csv"
    legacy_path = SENSITIVITY_DATA / "baseline_ewbi.csv"
    fallback_path = OUTPUT_DIR / "5_sensibility_test_local_uncertainty" / "data" / "baseline_ewbi.csv"

    for path, label in [
        (primary_path, "baseline"),
        (legacy_path, "legacy baseline"),
        (fallback_path, "fallback baseline"),
    ]:
        if path.exists():
            df = pd.read_csv(path)
            print(f"[OK] Loaded {label} data: {len(df):,} records")
            return df

    print(f"[ERROR] Baseline data not found at {primary_path} or {legacy_path} or {fallback_path}")
    return None
...
def load_experiment_data():
    ...
    for exp_file in experiment_files:
        ...
        # Load corresponding params/config (naming differs across versions)
        params_path = SENSITIVITY_DATA / f"experiment_{exp_num}_params.json"
...
def get_parameter_names(configs):
    ...
    # For data treatment sensitivity analysis - only data treatment variables
    DATA_TREATMENT_VARIABLES = {
        'BREAK_THRESHOLD',
        'APPLY_MOVING_AVERAGE',
        'MOVING_AVERAGE_WINDOW',
        'APPLY_MEAN_RESCALING',
        'LEVEL2_BREAK_ADJUSTMENT',
    }
    
    param_names = set()
    for cfg in configs.values():
        for key in cfg.keys():
            if key not in CONFIG_META_KEYS and key in DATA_TREATMENT_VARIABLES:
                param_names.add(key)
...
def compute_first_order_indices(y_values, param_values):
    ...
    # `param_values` can be mixed types across experiments (e.g., bool + None).
    # np.unique() sorts values, which fails for non-orderable mixtures.
    # Treat parameters as categorical by normalizing to strings.
    normalized = []
    ...
    param_values = np.asarray(normalized, dtype=object)
...
def plot_eu27_ewbi_all_experiments(baseline_df, experiments, configs, output_path):
    ...
    for i, (exp_name, exp_df) in enumerate(experiments.items()):
        ...
        if exp_ts is not None:
            ...
            if config.get('LEVEL2_BREAK_ADJUSTMENT') is False:
                label_parts.append('L2BreakOff')
...
def plot_config_impact_heatmap(baseline_df, experiments, configs, output_path):
    ...
    params_to_plot = [
        ...
        ('APPLY_MOVING_AVERAGE', 'Moving Average'),
        ('MOVING_AVERAGE_WINDOW', 'Moving Avg Window'),
        ('APPLY_MEAN_RESCALING', 'Mean Rescaling'),
        ('LEVEL2_BREAK_ADJUSTMENT', 'Level 2 Break Adjustment'),
...
def plot_experiment_comparison_table(baseline_df, experiments, configs, output_path):
        ...
        if exp_ts is not None:
            ...
            if len(exp_latest) > 0:
                ...
                rows.append({
                    ...
                    'MVG Avg Window': config.get('MOVING_AVERAGE_WINDOW', 'N/A'),
                    'Mean Rescaling': 'Yes' if config.get('APPLY_MEAN_RESCALING') else 'No',
                    'Level 2 Break Adjustment': 'Off' if config.get('LEVEL2_BREAK_ADJUSTMENT') is False else 'On',
    ...
    plt.title('Sensitivity Analysis: Data Treatment Variables Comparison', 
