...
1. Load and clean data from multiple source files
...
def create_measure_dataframe(df, measure_name, value_column):
    ...
    return measure_df[['ISO3', 'Country', 'Year', 'Measure', 'Value']]
...
def calculate_cumulative_emissions(df):
    ...
    df['Cumulative_CO2_emissions_Mt'] = df.groupby(['ISO3', 'Region'])['CO2_emissions_Mt'].cumsum()
    df['Cumulative_Consumption_CO2_emissions_Mt'] = df.groupby(['ISO3', 'Region'])['Consumption_CO2_emissions_Mt'].cumsum()
    return df

def create_aggregates(df, group_cols, agg_name, iso3_code, region_name):
    ...
    for region in emissions_df['Region'].unique():
        if pd.notna(region):  # Skip if region is null
            region_agg = create_aggregates(
                ...
                region,  # Use region name as ISO3
